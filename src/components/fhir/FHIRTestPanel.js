  ...
  const runTests = async () => {
    ...
    const tests = [
      ...
      },
      {
        ...
        test: async () => {
          try {
            const patients = await restApiService.searchResources('Patient', { _count: 1 });
            return { success: true, message: `Found ${patients.length} patient(s)` };
          } catch (error) {
        ...
        test: async () => {
          try {
            ...
            if (patients.length > 0) {
              const validation = fhirValidator.validateFHIRResource(patients[0].resource);
              return { 
            ...
            return { success: false, message: 'No resources to validate' };
          } catch (error) {
